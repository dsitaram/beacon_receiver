    ...
    for r in f["table"]:
        ...
        if os.path.exists(fname) and (r["name"] == "COSMOS_2407" or r["name"] == "COSMOS_2463"):
            try:
                ...
                (offset, power, npower) = estimate_offset(z,efname,ntime=30,window=8192,sr=r["sr0"],freq=r["freq0"]*1e6,offsets=numpy.arange(-50,50),t0=r["t0"],name=r["name"])
                if npower > 30.0:
                    ngood+=1
...
#    print(S.shape)
    ...
    df = sr/float(window)
    doppler_idx = numpy.array(numpy.round((doppler_prediction + (sr/2.0))/df),dtype=numpy.int)

    midx=numpy.arange(S.shape[0]/10)*10
#    plt.pcolormesh(midx,numpy.arange(window),numpy.transpose(10.0*numpy.log10(S[midx,:])))
    t_idx = numpy.array(numpy.round((tvec-t0)*sr2),dtype=numpy.int)
...
#        print("i %d o %d"%(i,o))
        doppler_prediction = -e["vel2"](tvec)*freq/scipy.constants.c 
        doppler_idx = numpy.array(numpy.round((doppler_prediction + (sr/2.0))/df + o),dtype=numpy.int)
