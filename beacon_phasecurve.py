...
def read_flights(dname):
    ...
    f.close()
    return({"idx":numpy.array(idx,dtype=int),"table":results})
...
def get_quicklook_spectrum(d,window=4096*2,n_window=500,bandwidth=300,ntfilt=25):
    offset_ppm = 0.0
    if c.use_measured_freq_offset:
        try:
            h=h5py.File("%s/offset.h5"%(c.datadir),"r")
            offset_ppm = h["offset_ppm"].value
            h.close()
        except:
            pass
        
    sr = d["f"]["sr0"]        
    correlation_step = window
    step = int(numpy.floor((len(d["z0"]) - correlation_step - 2.0*window)/n_window))
    res0 = numpy.zeros([n_window,window],dtype=numpy.complex64)
    res1 = numpy.zeros([n_window,window],dtype=numpy.complex64)
    res1f = numpy.zeros([n_window,window],dtype=numpy.complex64)
  #  res1k = numpy.zeros([n_window,window],dtype=numpy.complex64)

    # filter in time by 50 points
#    res1k[0:50,0]=1.0
 #   res1k=numpy.fft.fft2(res1k)
    ...
    fidx = numpy.arange(-bandwidth/2,bandwidth/2,dtype=int) + window/2

    for i in range(n_window):
        tvec[i]=float(i*step)/sr + d["f"]["t0"]
        z00 = d["z0"][i*step + idx]
        z01 = d["z0"][i*step + idx + correlation_step]
        z10 = d["z1"][i*step + idx]
        z11 = d["z1"][i*step + idx + correlation_step]
        # evaluate Doppler
        doppler0 = 1e6*d["f"]["freq0"]*(d["e"]["vel2"](0.5*(float(i*step)+float(i*step + correlation_step))/sr + d["f"]["t0"]))/scipy.constants.c + d["f"]["freq0"]*offset_ppm
        doppler1 = 1e6*d["f"]["freq1"]*(d["e"]["vel2"](0.5*(float(i*step)+float(i*step+correlation_step))/sr + d["f"]["t0"]))/scipy.constants.c + d["f"]["freq1"]*offset_ppm
        
        F0 = numpy.fft.fftshift(numpy.fft.fft(wfun*z00*numpy.exp(1.0j*2.0*numpy.pi*doppler0*(idx/sr))))
        F1 = numpy.fft.fftshift(numpy.fft.fft(wfun*z01*numpy.exp(1.0j*2.0*numpy.pi*doppler0*(idx/sr + float(correlation_step)/sr))))
        # correlate in time
        res0[i,:]=F0*numpy.conj(F1)
        
        F0 = numpy.fft.fftshift(numpy.fft.fft(wfun*z10*numpy.exp(1.0j*2.0*numpy.pi*doppler1*(idx/sr))))
        F1 = numpy.fft.fftshift(numpy.fft.fft(wfun*z11*numpy.exp(1.0j*2.0*numpy.pi*doppler1*(idx/sr+float(correlation_step)/sr))))
        res1[i,:]=F0*numpy.conj(F1)
        
        res0[i,:]=res0[i,:]/numpy.median(numpy.abs(res0[i,:]))
        res1[i,:]=res1[i,:]/numpy.median(numpy.abs(res1[i,:]))

    # time median filter out rfio
    for i in range(n_window/ntfilt):
        for j in range(len(fidx)):
            res1f[ntfilt*i+numpy.arange(ntfilt),fidx[j]]=numpy.median(numpy.abs(res1[ntfilt*i+numpy.arange(ntfilt),fidx[j]]))
    
    for i in range(n_window):
        freqm[i] = fvec[fidx[numpy.argmax(res1f[i,fidx])]]
...
#            print("%d/%d"%(i,n_points))
        ...
        for k in range(incoh_int):
            ...
            doppler0 = -1.0*(150.0/400.0)*ql["doppler_residual"](tdop) + 1e6*d["f"]["freq0"]*d["e"]["vel2"](tdop)/scipy.constants.c + d["f"]["freq0"]*offset_ppm
            doppler1 = -1.0*ql["doppler_residual"](tdop) + 1e6*d["f"]["freq1"]*d["e"]["vel2"](tdop)/scipy.constants.c + d["f"]["freq1"]*offset_ppm
...
def analyze_flight(dname,flight):
    ...
    if dur > c.min_duration:
        if os.path.exists("%s/flight-%06d.pass"%(dname,flight)):
            d = read_data(dname=dname,flight=flight)
            d["pc"] = estimate_phase_curve(d)
            save_result(d)
            plot_overview(d)
        else:
            print("no pass file, not analyzing %d"%(flight))
        f = open("%s/nanalyzed.log"%(dname),"a")
        f.write("%d\n"%(flight))
        f.close()
    else:
        print("too short flight. not analyzing.")
#    return(d)
...
def analyze_all(maxn=10):
    ...
    for d in dl:
        ...
        if os.path.exists("%s/flights.log"%(d)):
            ...
            for i in f["idx"]:
                # if analyzed, skip
                if not numpy.in1d(numpy.array([i],dtype=int),an_idx):
                    if verbose_debug:
...
def save_result(d):
    ...
    fname = "%s/%d@%s@%s.h5"%(d["dname"],int(d["f"]["t0"]),d["f"]["name"],c.station)
...
def estimate_offsets(d,channel=0,plot=False):
    f = read_flights(d)
    avg = 0.0
    var_sum = 0.0
    ngood = 0
    for r in f["table"]:
        if channel == 0:
            fname = "%s/flight-%06d.001.bin"%(d,r["idx"])
        else:
            fname = "%s/flight-%06d.002.bin"%(d,r["idx"])
        efname = "%s/flight-%06d.pass"%(d,r["idx"])
        if os.path.exists(fname) and (r["name"] == "COSMOS_2407" or r["name"] == "COSMOS_2463"):
            try:
                #            print("name %s %1.2f %s %1.2f %d"%(r["name"],r["t0"],fname,r["freq0"],r["idx"]))
                z = numpy.fromfile(fname,dtype=numpy.complex64)
                if channel==0:
                    (offset, power, npower) = estimate_offset(z,efname,ntime=30,window=8192,sr=r["sr0"],freq=r["freq0"]*1e6,offsets=numpy.arange(-1000,1000),t0=r["t0"],name=r["name"])
                else:
                    (offset, power, npower) = estimate_offset(z,efname,ntime=30,window=8192,sr=r["sr0"],freq=r["freq1"]*1e6,offsets=numpy.arange(-1000,1000),t0=r["t0"],name=r["name"])   
                    
                if npower > 30.0:
                    ngood+=1
                var_sum += 10.0**(npower/10.0)
                avg += 10.0**(npower/10.0)*offset
                print("avg %1.5f ppm"%(avg/var_sum))
                if ngood > 3:
                    return( avg/var_sum )
            except:
                pass
...
def estimate_offset(z,efname,ntime=20,window=8192,sr=40e3,freq=149.94e6,offsets=numpy.arange(-150,150),t0=0,name="NONE"):
    e = read_ephem(efname)
#    print(t0)
...
#    print(S.shape)
    ...
    tvec = numpy.linspace(t0+10,(len(z)/sr + t0)-10,num=100)
    doppler_prediction = -e["vel2"](tvec)*freq/scipy.constants.c
    df = sr/float(window)
    doppler_idx = numpy.array(numpy.round((doppler_prediction + (sr/2.0))/df),dtype=int)

    midx=numpy.arange(S.shape[0]/10)*10
#    plt.pcolormesh(midx,numpy.arange(window),numpy.transpose(10.0*numpy.log10(S[midx,:])))
    t_idx = numpy.array(numpy.round((tvec-t0)*sr2),dtype=int)
 #   plt.plot(t_idx,doppler_idx,".")
 #   plt.show()

    p = numpy.zeros([len(offsets)])
    
    for i,o in enumerate(offsets):
#        print("i %d o %d"%(i,o))
        doppler_prediction = -e["vel2"](tvec)*freq/scipy.constants.c 
        doppler_idx = numpy.array(numpy.round((doppler_prediction + (sr/2.0))/df + o),dtype=int)
        
        p[i]=numpy.sum(S[t_idx,doppler_idx])

    mi = numpy.argmax(p)
    ppm = -1e6*(float(offsets[mi])*df)/freq
    mp = numpy.median(p)
    print("%s offset %1.5f ppm power %1.0f dB"%(name, ppm, 10.0*numpy.log10(p[mi]/mp)))
    return((ppm,10.0*numpy.log10(p[mi]),10.0*numpy.log10(p[mi]/mp)))
            
def update_offsetfile():
    
    tnow = time.time()
    t = time.gmtime(tnow-2*24.0*3600.0)
    prevday = "%04d.%02d.%02d"%(t.tm_year,t.tm_mon,t.tm_mday)
    print(prevday)
    data_dir = "%s/%s"%(c.datadir,prevday)
    print(data_dir)

    ppm = estimate_offsets(data_dir)
    
    print("ppm %1.2f"%(ppm))
    h = h5py.File("%s/offset.h5"%(c.datadir),"w")
    h["offset_ppm"]=ppm
    h["t"]=time.time()
    h.close()
