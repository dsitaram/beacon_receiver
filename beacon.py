...
# 
#
import optparse, os, time, _thread, datetime
    ...
    for tnow in times_to_check:
        ...
        if not os.path.exists("%s/%s/passes.txt"%(c.datadir,date_string)):
            ...
            beacon_predict.write_pass_files(datestr="%04d/%02d/%02d 00:00:00"%(tnow.year,tnow.month,tnow.day),
                                            ...
                                            year=tnow.year, month=tnow.month, day=tnow.day)
...
def do_housekeeping(state):
    # Get satellite ephemeris from web, check recorder status
    ...
    if len(last_lines) < 1:
        ...
        status_string = "%s tle %d recorder alive %d last analyzed %s %s" % \
            (c.station, state["last_tle_fetch"],state["rec_alive"], last_an, last_lines[0].strip())
    print(status_string)
    with open("%s/status.log"%(c.datadir),"w") as status_file:
...
def cleanup():
    while True:
        ...
        if c.use_measured_freq_offset:
            beacon_phasecurve.update_offsetfile()
...
def run_beacon_receiver():
    ...
    cmd = "LC_ALL=\"C\"; thor.py -o %s -m %s -d %s -f %i -b %i -g %i -y %s -duration=%d > %s/rout.log 2> %s/rerr.log" % (c.datadir, c.uhd_device_address, c.uhd_subdev_string, 400e6, c.uhd_decimation, c.uhd_gain1, "TX/RX", c.uhd_refclock, c.datadir, c.datadir)
...
if __name__ == "__main__":
    ...
    _thread.start_new_thread(run_beacon_receiver, ())
    _thread.start_new_thread(run_beacon_calc, ())
