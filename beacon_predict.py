...
class satellitedata:
    ...
    def readData(self):
        ...
        print("number of satellites: %i starting search at %f" % (self.nsatellites,self.startd))
        for i in range(0,self.nsatellites):
            ...
            print("%s  %f ppm" % (self.name[i], self.ppm[i]))
        print("done")
        ...
        for si in range(0,self.nsatellites):
            ...
            for i in range(1,int(60*60*24.5),10):
                ...
                if el > elcutoff and i < 60.0*60.0*24.0 and satUp == 0:
                    ...
                    print("%i rise %s %f" % (passidx,self.name[si],risetime))
#                    print time.ctime(risetime)
                ...
                if el < elcutoff and satUp == 1:
                    ...
                    print("set  %s %f peak elevation %f\n" % (self.name[si],settime,peak))
                    ...
                    passidx = passidx + 1
                if satUp == 1:
                    with open("%s/flight-%06d.pass"%(self.ddir,passidx),"w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_option('-y', '--year', dest='year', type="int", default=2015)
    parser.add_option('-m', '--month', dest='month', type="int", default=9)
    parser.add_option('-n', '--day', dest='day', type="int", default=5)
    parser.add_option('-d', '--dir', dest='dir', type="string", default="/data/beacon/data/2021.07.01/")
